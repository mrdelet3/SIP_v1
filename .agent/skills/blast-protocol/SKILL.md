...
name: blast-protocol
description: >
  The B.L.A.S.T. (Blueprint, Link, Architect, Stylize, Trigger) master protocol
  for building deterministic, self-healing automation systems. Use when the user
  initiates a new automation project, multi-phase build, or any task requiring
  persistent project memory, external API integrations, and structured delivery.
---
...
**3. Research:** Search GitHub repos and other resources for helpful libraries or patterns for the project.
...
**2. Handshake:** Build minimal scripts in `tools/` to verify that external services are responding. Do not proceed to full logic if the "Link" is broken.
...
Before building any Tool, define the **Data Schema** in `gemini.md`:
...
4. **Update Architecture**: Record the new learning in the corresponding `architecture/` SOP so the error never repeats.

### 3. Deliverables vs. Intermediates

- **Local (`.tmp/`):** Scraped data, logs, and temporary files. Ephemeral.
